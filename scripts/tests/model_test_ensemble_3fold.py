...
for irun in np.arange(100):
    ...
    while again == "y":
        ...
        for l in np.arange(0, len(A_tot)):
            A_exp_add.append([j for j in A_exp[l][0]])
            A_exp_rem.append([j for j in A_exp[l][1]])

        # calculate the administrators created by the mechanism
        for m in np.arange(0, len(A_tot)):
            A_shk.append(list(set(A_tot[m]).difference(set(A_tot[m-1])).difference(A_exp[m][0])))
            A_shk4 = A_shk4.difference(A_exp_rem[m])
            A_shk4 = A_shk4.union(A_shk2[m])
            A_shk4_size.append(len(A_shk4))
            A_exp_temp = A_exp_temp.union(set(A_exp_add[m]))
            A_exp_temp = A_exp_temp.difference(set(A_exp_rem[m]))
        ...
        if runmode == "e":
            ...
            dat.to_csv("../../results/model/"+folder+"/t5_explore"+str(irun)+".csv")
            # print("data saved to exploration")
        elif runmode == "i":
            data_intermediate = data.copy()
            dat = pd.DataFrame(data_intermediate)
            dat.to_csv("../../results/model/"+folder+"/t5_intermediate"+str(irun)+".csv")
            # print("data saved to intermediate")
        elif runmode == "b":
            data_base = data.copy()
            dat = pd.DataFrame(data_base)
            dat.to_csv("../../results/model/"+folder+"/t5_base"+str(irun)+".csv")
